...
const App: React.FC = () => {
  const { user, loading: authLoading, isAuthenticated, otpSent, startPhoneSignIn, confirmCode, signOut, refreshUser } = useAuth();
...
//   const { location, loading: getCurrentPosition } = useLocation();
//   const { 
...
//   };
  ...
  // Show role selection only if user hasn't selected a role yet
  if (isAuthenticated && user && !user.hasSelectedRole) {
    return (
      <SafeAreaView style={globalStyles.container}>
        <RoleSelection User={user} onRoleSelected={refreshUser} />
      </SafeAreaView>
    );
  }
  ...
  // default to rider
  return <RiderDashboard user={user!} onSignOut={signOut} />;
};
